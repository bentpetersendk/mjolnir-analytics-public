...
Opportunities/Cost Insights) is produced upstream and synced into this
public repo as a snapshot - there is no generator script for it here. The
frontend already reads `collector`, `collector_status`, `platform_module`,
and `data_window_days` from it (`js/data-loader.js`) with graceful
fallbacks, so once its upstream generator adopts this contract, the
Platform Status panel reflects it automatically with **no frontend
change required**. Until then, its health is derived purely from
`generated_at`'s age. (The Infrastructure/Nodes/Hardware/Capacity pages'
`node_insights.json` is generated by this repo's own
`scripts/export_node_insights.py`, covered above - it's not in this
upstream-snapshot category.)
